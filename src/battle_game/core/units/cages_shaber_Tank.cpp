...
cages_shaber_Tank::cages_shaber_Tank(GameCore *game_core,
                                     uint32_t id,
                                     uint32_t player_id)
...
void cages_shaber_Tank::TankMove(float move_speed, float rotate_angular_speed) {
...
void cages_shaber_Tank::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
...
const char *cages_shaber_Tank::UnitName() const {
  return "cage's_shaber_Tank";
}

const char *cages_shaber_Tank::Author() const {
  return "cage";
}
}  // namespace battle_game::unit

...
cages_apple_tank::cages_apple_tank(GameCore *game_core,
                                   uint32_t id,
                                   uint32_t player_id)
...
void cages_apple_tank::Update() {
  TankMove(3.0f, glm::radians(180.0f));
  TurretRotate();
  Fire();
}

void cages_apple_tank::TankMove(float move_speed, float rotate_angular_speed) {
...
void cages_apple_tank::TurretRotate() {
  auto player = game_core_->GetPlayer(player_id_);
  if (player) {
    auto &input_data = player->GetInputData();
